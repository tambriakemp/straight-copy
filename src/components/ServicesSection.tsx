    ...
    <section id="services" className="py-[120px] md:py-[160px] bg-warm-white">
      <div className="max-w-[1400px] mx-auto px-7 md:px-16">
        ...
        <div className="max-w-[960px] mx-auto text-center mb-20 md:mb-24 reveal">
          ...
          <p className="text-[21px] font-light leading-[1.75] text-taupe max-w-[680px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[2px] bg-mist-custom">
          {automations.map((a, i) => (
            ...
            >
              <div className="text-[13px] font-medium tracking-[0.28em] uppercase text-taupe mb-12 transition-colors duration-500 group-hover:text-stone">
                {a.num}
              </div>
              <h3 className="font-serif text-[34px] md:text-[42px] font-light text-ink mb-7 leading-[1.08] transition-colors duration-500 group-hover:text-warm-white">
                {renderTitle(a.title)}
              </h3>
              <p className="text-[19px] font-light leading-[1.75] text-charcoal mb-10 transition-colors duration-500 group-hover:text-stone">
                {a.desc}
              </p>
              <ul className="list-none">
                {a.steps.map((step, idx) => (
                  <li
                    key={idx}
                    className="text-[17px] font-light leading-[1.6] text-charcoal py-4 border-t border-mist-custom grid grid-cols-[20px_1fr] gap-3.5 transition-colors duration-500 group-hover:text-stone group-hover:border-warm-white/[0.08]"
                  >
                    <span className="text-accent text-[16px] leading-[1.7]">→</span>
                    <span>{step}</span>
                  </li>
                ))}
              </ul>
        ...
        <div className="mt-20 bg-cream p-10 md:p-[64px] md:py-[72px] grid grid-cols-1 md:grid-cols-[1fr_1.4fr] gap-10 md:gap-[72px] items-center border-t-2 border-accent reveal">
          <div>
            <div className="font-serif text-[30px] md:text-[34px] font-light text-ink mb-4 leading-[1.15]">
              Need something more?
            </div>
            <p className="text-[18px] font-light text-charcoal leading-[1.75]">
          ...
          <div className="flex flex-wrap gap-2.5">
            {upchargeTags.map((tag) => (
              <span
                key={tag}
                className="text-[14px] font-medium tracking-[0.14em] uppercase text-charcoal bg-warm-white border border-sand px-5 py-3"
              >
                {tag}
              </span>
            ))}
