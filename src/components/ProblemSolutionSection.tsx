  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-2 items-stretch">
      ...
      <div className="bg-cream px-8 md:px-[72px] py-24 md:py-[140px]">
        <p className="inline-flex items-center gap-3.5 text-[13px] font-medium tracking-[0.35em] uppercase text-taupe mb-10 reveal before:content-[''] before:w-9 before:h-px before:bg-taupe">
          The Problem
        </p>
        <h2
          className="font-serif font-light leading-[1.1] text-ink mb-16 reveal"
          style={{ fontSize: "clamp(42px, 4.2vw, 60px)", letterSpacing: "-0.01em" }}
        >
        ...
        <ul className="list-none">
          {problems.map((p, i) => (
            <li
              key={i}
              className="text-[19px] font-light leading-[1.7] text-charcoal py-6 border-b border-mist-custom grid grid-cols-[32px_1fr] gap-5 items-start reveal"
            >
              <span className="text-taupe text-[16px] leading-[1.7] mt-0.5">✕</span>
              <span>{p}</span>
            </li>
          ))}
        </ul>
      ...
      <div className="bg-ink px-8 md:px-[72px] py-24 md:py-[140px]">
        <p className="inline-flex items-center gap-3.5 text-[13px] font-medium tracking-[0.35em] uppercase text-accent mb-10 reveal before:content-[''] before:w-9 before:h-px before:bg-accent">
          The Architecture
        </p>
        <h2
        ...
        <ul className="list-none">
          {solutions.map((s, i) => (
            <li
              key={i}
              className="text-[19px] font-light leading-[1.7] text-stone py-6 border-b border-warm-white/[0.08] grid grid-cols-[32px_1fr] gap-5 items-start reveal"
            >
              <span className="text-accent text-[16px] leading-[1.7] mt-0.5">→</span>
              <span>{s}</span>
            </li>
          ))}
        </ul>
