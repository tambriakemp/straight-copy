...
export default function AiEditDialog({ open, onOpenChange, projectId, pagePath, onApplied }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !busy && onOpenChange(v)}>
      ...
      >
        <DialogHeader>
          <DialogTitle style={{ display: "flex", alignItems: "center", gap: 10, fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 300, fontSize: 28, color: "hsl(40 20% 97%)" }}>
            <Sparkles size={18} style={{ color: "hsl(30 25% 44%)" }} />
            Edit page with AI
          </DialogTitle>
          <DialogDescription style={{ fontSize: 17, color: "hsl(30 8% 62%)", fontFamily: "monospace", marginTop: 4 }}>{pagePath}</DialogDescription>
        </DialogHeader>

        <div style={{ display: "grid", gap: 16, marginTop: 8 }}>
          {/* Attachments */}
          <div>
            <div style={{ fontSize: 16, letterSpacing: "0.3em", textTransform: "uppercase", color: "var(--crm-taupe)", marginBottom: 8 }}>
            ...
            >
              {attachments.length === 0 ? (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, color: "var(--crm-taupe)", fontSize: 18 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 10 }}>
                  {attachments.map((a, i) => (
                    <div key={i} style={{ position: "relative", borderRadius: 6, overflow: "hidden", border: "1px solid var(--crm-border-dark)" }} onClick={(e) => e.stopPropagation()}>
                      ...
                      <div style={{ padding: "4px 6px", fontSize: 15, color: "var(--crm-taupe)", fontFamily: "monospace", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.targetPath}</div>
                    </div>
                  ))}
                  <button
                    onClick={(e) => { e.stopPropagation(); fileInput.current?.click(); }}
                    style={{ minHeight: 90, border: "1px dashed var(--crm-border-dark)", borderRadius: 6, background: "transparent", color: "var(--crm-taupe)", cursor: "pointer", fontSize: 17 }}
                  >
                    + Add more
                  </button>
                </div>
          ...
          <div>
            ...
            />
            <div style={{ marginTop: 6, fontSize: 16, color: "var(--crm-taupe)" }}>
