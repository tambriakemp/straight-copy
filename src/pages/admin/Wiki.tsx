...
const eyebrow: React.CSSProperties = { fontSize: 16, letterSpacing: "0.35em", textTransform: "uppercase", color: TAUPE, marginBottom: 16 };
const title: React.CSSProperties = { fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, fontSize: 60, lineHeight: 1.05, margin: 0, letterSpacing: "-0.01em" };
...
const sub: React.CSSProperties = { color: TAUPE, fontSize: 19, lineHeight: 1.7, margin: 0, maxWidth: 620 };
const sectionLabel: React.CSSProperties = { fontSize: 15, letterSpacing: "0.3em", textTransform: "uppercase", color: STONE, marginBottom: 14 };

const btn: React.CSSProperties = {
  background: "transparent", border: `1px solid ${BORDER}`, color: CREAM,
  padding: "10px 18px", fontSize: 15, letterSpacing: "0.2em", textTransform: "uppercase",
...
const input: React.CSSProperties = {
  ...
  color: CREAM, padding: "10px 14px", fontSize: 18, fontFamily: "inherit", outline: "none",
};
...
function Badge({ children, tone = STONE }: { children: React.ReactNode; tone?: string }) {
  return (
    <span style={{
      display: "inline-block", padding: "3px 10px", fontSize: 14, letterSpacing: "0.18em",
        ...
        <div style={{ border: `1px solid ${BORDER}`, background: CHARCOAL }}>
          {!isMobile && (
            <div style={{
              ...
              fontSize: 14, letterSpacing: "0.25em", textTransform: "uppercase", color: STONE,
            }}>
          ...
          {!loaded ? <div style={{ padding: 32, color: TAUPE }}>Loading…</div> :
           ...
           filtered.map(d => (
            <div key={d.id} onClick={() => nav(`/admin/wiki/${d.slug}`)} style={{
              ...
              <div>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: isMobile ? 22 : 24, fontWeight: 300, color: CREAM, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  {d.title}
                  {isStale(d.last_reviewed_at) && <span title="Not reviewed in 6+ months" style={{ color: "hsl(8 55% 70%)", fontSize: 15, letterSpacing: "0.2em", textTransform: "uppercase" }}>· stale</span>}
                  {d.access_level === "Founder Only" && <span title="Founder Only" style={{ color: ACCENT, fontSize: 14 }}>● founder</span>}
                </div>
                {d.tags.length > 0 && <div style={{ fontSize: 16, color: TAUPE, marginTop: 4 }}>{d.tags.map(t => `#${t}`).join(" ")}</div>}
              </div>
              <div style={{ fontSize: 16, color: TAUPE, letterSpacing: "0.05em" }}>{d.department}</div>
              <div><Badge>{d.doc_type}</Badge></div>
              <div><Badge tone={statusTone(d.status)}>{d.status}</Badge></div>
              <div style={{ fontSize: 16, color: TAUPE }}>{fmtDate(d.updated_at)}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <AdminLayout>
      <div style={pageScroll}><div style={page}>
        ...
        <input
          ...
          style={{ ...input, fontFamily: "'Cormorant Garamond', serif", fontSize: 46, fontWeight: 300, padding: "12px 0", border: 0, borderBottom: `1px solid ${BORDER}`, marginBottom: 32 }}
        />
        ...
        {isSop ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
            {SOP_SECTIONS.map(s => (
              <div key={s.key}>
                <div style={{ ...sectionLabel, marginBottom: 10, color: CREAM, fontFamily: "'Cormorant Garamond', serif", fontSize: 24, fontWeight: 300, letterSpacing: 0, textTransform: "none" }}>
                  {s.heading}
                </div>
                <WikiEditor
      ...
      <div style={pageScroll}><div style={isMobile ? { ...page, padding: "24px 16px 80px" } : page}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 280px", gap: isMobile ? 24 : 48, marginTop: 16 }}>
          <div>
            ...
            <h1 style={isMobile ? { ...title, fontSize: 38 } : title}>{doc.title}</h1>
            <hr style={rule} />
          ...
          <aside style={{ background: CHARCOAL, border: `1px solid ${BORDER}`, padding: 24, alignSelf: "start" }}>
            ...
            <dl style={{ margin: 0, fontSize: 17 }}>
              {[
                ...
                <div key={k as string} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "8px 0", borderBottom: `1px solid ${BORDER}` }}>
                  <dt style={{ color: STONE, fontSize: 15, letterSpacing: "0.2em", textTransform: "uppercase" }}>{k}</dt>
            ...
            {isStale(doc.last_reviewed_at) && (
              <div style={{ marginTop: 20, padding: 12, border: `1px solid hsl(8 55% 70% / 0.4)`, color: "hsl(8 55% 80%)", fontSize: 16 }}>
        ...
        <div style={{ marginTop: 32, border: `1px solid ${BORDER}`, background: CHARCOAL }}>
          {revs.length === 0 ? (
            ...
            <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, padding: "16px 24px", borderBottom: `1px solid ${BORDER}` }}>
              <div>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, color: CREAM }}>{r.title}</div>
                <div style={{ fontSize: 16, color: TAUPE, marginTop: 4 }}>
            ...
            <div onClick={e => e.stopPropagation()} style={{ background: INK, border: `1px solid ${BORDER}`, padding: 40, maxWidth: 900, margin: "auto", width: "100%" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
                <div>
                  ...
                  <h2 style={{ ...title, fontSize: 40 }}>{viewing.title}</h2>
                </div>
          ...
          {load2 ? <div style={{ padding: 24, color: TAUPE }}>Loading…</div> :
           ...
           rows.map(r => (
            <div key={r.id} style={{ display: "grid", gridTemplateColumns: "1.4fr 1.4fr 1fr 90px 200px", gap: 16, padding: "16px 24px", borderBottom: `1px solid ${BORDER}`, alignItems: "center" }}>
              <div><div style={{ color: CREAM, fontSize: 18 }}>{r.name}</div><div style={{ fontSize: 15, color: STONE, letterSpacing: "0.05em" }}>{r.user_id}</div></div>
              <div style={{ color: TAUPE, fontSize: 17 }}>{r.email}</div>
